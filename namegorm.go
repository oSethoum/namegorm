...
type NamingStrategy struct {
	...
	NoLowerCase    bool
	ColumnNameFunc func(column string) string
	ColumnNameCase Case
	TableNameFunc  func(table string) string
	TableNameCase  Case
}
...
func (ns NamingStrategy) ColumnName(table, column string) string {
	...
	if ns.ColumnNameCase != DefaultCase {
		switch ns.ColumnNameCase {
		case CamelCase:
			if strings.ToLower(column) == "id" {
				...
				bts[0] = bytes.ToLower([]byte{bts[0]})[0]
				column = string(bts)
				if strings.HasSuffix(column, "ID") {
...
func (ns NamingStrategy) toSchemaName(name string) string {
	result := strings.ReplaceAll(strings.ToTitle(strings.ReplaceAll(name, "_", " ")), " ", "")
	for _, initialism := range commonInitialisms {
		result = regexp.MustCompile(strings.ToTitle(strings.ToLower(initialism))+"([A-Z]|$|_)").ReplaceAllString(result, initialism+"$1")
	}
	return result
}
